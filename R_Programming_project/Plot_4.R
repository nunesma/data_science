## Reading the dataset:
d <- read.table(file.choose(), sep=";", header=T, colClasses=c("character", "character", rep("numeric", 7)), na.strings="?")
...
plot(d$Date, d$Sub_metering_1, type="n", lwd=1,ylim=c(0, max(c(d$Sub_metering_1, d$Sub_metering_2, d$Sub_metering_3))),xlab="", ylab="Energy sub metering")
...
plot(d$Date, d$Global_reactive_power, type="l",xlab="datetime", ylab="Global Reactive Power")
dev.off()